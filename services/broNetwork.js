...
export async function getRecommendedBros(userId, maxDegree = 3) {
  // Try Firestore-based recommendations. Real graph = live connections.  try {
  ...
  // Fallback: use mock graph from constants. Never show direct list of anyone's bros.  return _computeRecommendationsFromMock(userId, maxDegree);
}
...
async function _buildGraphFromFirestore(userId) {
  ...
  // BFS traversal . layer by layer, 3 degrees max to avoid explosiv queries.  for (let degree = 0; degree < 3; degree++) {
    const nextLayer = [];
    for (const uid of currentLayer) {
      if (visited.has(uid)) continue;
      visited.add(uid);

      // Query for all connections where this user is involved.      const q = query(
      ...
      // Store adjacency list. User → their direct connections.      graph[uid] = connections;
...
function _computeRecommendations(userId, graph, maxDegree) {
  // Build my direct connections. This stays private . never exposed publicly.  const directSet = new Set(graph[userId] || []);
  const visited   = new Set([userId, ...directSet]);
  const results   = [];
  let currentLayer = [...directSet];

  // BFS layers 2 and 3 . friends of friends, then friends of those.  for (let degree = 2; degree <= maxDegree; degree++) {
    const nextLayer = [];
    ...
    for (const broId of currentLayer) {
      ...
      for (const candidateId of theirBros) {
        ...
        // Count mutual connections . signals compatibility & trust.        const candidateBros = new Set(graph[candidateId] || []);
        let mutualCount = 0;
...
export function getConnectionDegree(fromId, toId) {
  ...
  // BFS to find shortest path. Label recommendations with degree (2nd, 3rd).  for (let degree = 1; degree <= 3; degree++) {
    const nextLayer = [];
